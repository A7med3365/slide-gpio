...
class SlideshowManager(threading.Thread):
    ...
    def __init__(self, folder_map, initial_folder_key, delay_seconds):
        ...
        self._next_image_files = [] # Buffer for next set of images
    ...
    def _start_feh(self, image_files=None):
        """Starts the feh slideshow subprocess with the specified image files or the default ones."""
        files_to_use = image_files if image_files is not None else self._image_files
        
        if not files_to_use:
            print("[Slideshow] No images provided for slideshow. Not starting feh.")
            return None
        ...
        current_key = self._current_folder_key if image_files is None else "new set"
        print(f"[Slideshow] Starting feh for folder key {current_key}...")
        try:
            ...
            print(f"[Slideshow] feh started with PID: {process.pid}")
            return process
    ...
    def run(self):
        ...
        while not self._shutdown_event.is_set():
            ...
            if folder_changed and key_to_load is not None:
                # First, find images in the new folder
                ...
                if folder_path:
                    ...
                    if next_images:
                        ...
                        new_process = self._start_feh(next_images)
                        if new_process:
                            # Stop old slideshow (if running) only after new one has started
                            if self._feh_process and self._feh_process.poll() is None:
                                self._stop_feh(self._feh_process)
                            
                            # Update current state to the new folder
                            self._image_files = next_images
                            self._feh_process = new_process
                            self._current_folder_key = key_to_load
                        else:
                            print(f"[Slideshow] Failed to start new slideshow for key {key_to_load}")
                    else:
                        print(f"[Slideshow] No images found for key {key_to_load}, not changing slideshow")
                        self._current_folder_key = key_to_load  # Still update the key
                else:
                    print(f"[Slideshow] Folder path not found for key {key_to_load}")
                    self._image_files = []
                    if self._feh_process:
                        self._stop_feh()
                    self._current_folder_key = key_to_load  # Still update the key
            ...
            if self._feh_process and self._feh_process.poll() is not None:
                 ...
                 if not self._shutdown_event.is_set() and self._image_files:
                     ...
                     self._feh_process = self._start_feh()
